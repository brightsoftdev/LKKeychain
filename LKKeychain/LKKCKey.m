...
- (BOOL)addToKeychain:(LKKCKeychain *)keychain error:(NSError **)error
{
    if (self.keychain != nil || self.keyClass != LKKCKeyClassSymmetric)
        return [super addToKeychain:keychain error:error];
    NSData *data = self.keyData;
    if (data == nil) {
        // Can't extract key; try SecItemAdd as last resort.
        return [super addToKeychain:keychain error:error];   
    }
    ...
    // However, it quite happily accepts a duplicate key created by SecKeyCreateFromData.
    ...
    [attributes setObject:[[self class] _algorithmFromLKKCKeyType:self.keyType] forKey:kSecAttrKeyType];
    [attributes setObject:[NSNumber numberWithInt:self.keySize] forKey:kSecAttrKeySizeInBits];
    if (self.canEncrypt)
    ...
    SecKeyRef skey = SecKeyCreateFromData((CFDictionaryRef)attributes, (CFDataRef)data, &cferror);
    if (skey == NULL) {
        LKKCReportError((OSStatus)CFErrorGetCode(cferror), error, @"Can't create key");
        return NO;
    }
    
    [attributes removeAllObjects];
    [attributes addEntriesFromDictionary:_updatedAttributes];
    [attributes setObject:[NSArray arrayWithObject:(id)skey] forKey:kSecUseItemList];
    [attributes setObject:[[self class] itemClass] forKey:kSecClass];
    [attributes setObject:(id)keychain.SecKeychain forKey:kSecUseKeychain]; // Private in 10.6
    ...
    if (CFGetTypeID(result) == CFArrayGetTypeID() && CFArrayGetCount(result) == 1) {
        CFRelease(_sitem);
        _sitem = (SecKeychainItemRef)CFArrayGetValueAtIndex(result, 0);
    }
    else if (CFGetTypeID(result) == SecKeyGetTypeID()) {
        CFRelease(_sitem);
        _sitem = (SecKeychainItemRef)result;
    }
    else {
        CFRelease(result);
        LKKCReportError(errSecInvalidKeyRef, error, @"Invalid result from SecItemAdd");
        return NO;
    }
    [self revertItem];
    return YES;
